...
// export default class Form1 extends React.Component<FormProps> {
...
const Form: React.FC<FormProps> = ({ handleFormSubmit }) => {
  ...
  return (
    <form className={styles.form} data-testid="form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <InputWrapper name="date" title="Date of birth" error={errors.date?.message}>
        <input
          {...register('date', { required: 'Value should not be empty' })}
          type="datetime-local"
          id="date"
        />
      </InputWrapper>
      ...
      >
        <input
          {...register('promo', { required: 'Value should not be empty' })}
          type="checkbox"
          id="promo"
        />
      </InputWrapper>
